...
namespace native_jvm::utils
{
    ...
    jobjectArray create_multidim_array(JNIEnv *env, jint count, jint required_count,
                                       const char *class_name, int line, std::initializer_list<jint> sizes, int dim_index)
    {
        ...
        std::string clazz_name = count == 1 ? std::string(class_name) : std::string(count - 1, '[') + "L" + std::string(class_name) + ";";
    ...
    {
        jobject _lookup = env->NewObject(methodhandles_lookup_class, lookup_init_method, clazz);
        if (env->ExceptionCheck())
        {
            return nullptr;
        }
        jobject lookup = env->NewGlobalRef(_lookup);
        env->DeleteLocalRef(_lookup);
        return lookup;
    }
