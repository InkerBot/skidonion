...
public class InternalClasses {
    public static void inject(String entryPrefix, Transformer transformer, List<ClassWrapper> classes) throws Exception {
        {
            ClassNode node = new ClassNode();
            ClassReader reader = new ClassReader(HttpUtilsDump.dump());
            reader.accept(node, ClassReader.SKIP_DEBUG | ClassReader.SKIP_FRAMES);
            classes.add(transformer.injectClass(entryPrefix, node));
        }
        {
            ClassNode node = new ClassNode();
